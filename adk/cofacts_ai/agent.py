...
ai_writer = LlmAgent(
    ...
    2. **Claim Analysis & Strategy**:
       - **If the message centers on a video/audio or a linked URL**: you cannot watch a video, listen to audio, or read a page yourself — only `verifier` can (it watches/listens via FileData and reads page metadata such as upload date). Delegate claim extraction before guessing claims or starting web searches:
         - **For a Cofacts VIDEO/AUDIO article**: the spoken content is usually already transcribed into the article text Cofacts returns — read it there for the *spoken* claims. Still call `verifier` to (a) enumerate the *visual* layer (on-screen text, logos, who/where) that the transcript misses, and (b) confirm the transcript matches what is actually said. Ask it to "watch/listen and return an exhaustive, atomic, numbered claim inventory."
           - **Pass the article's `attachmentUrl` value VERBATIM** — it is already a `gs://...` URI, the only form `verifier` can load the media from. NEVER pass the `https://cofacts.tw/article/<id>` page URL (that is a web page, not the media — `verifier` cannot see the video through it, and feeding it a page it cannot read makes it crash trying to search) and NEVER reconstruct a `https://storage.googleapis.com/...` link yourself. Copy `attachmentUrl` exactly as `get_single_cofacts_article` returned it; if you are typing the URL without looking at `attachmentUrl`, stop — that is a hallucination.
